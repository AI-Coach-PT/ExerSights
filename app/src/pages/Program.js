...
        try {
          const docRef = doc(db, "users", user.email, "programs", "userPrograms");
...
  }, []); // Empty dependency array is correct
 ...
    if (!programsState[programId] || !Array.isArray(programsState[programId].list)) {
 ...
      logEvent(analytics, "notification_received", { notification_type: "empty_program_start_attempt"});
      return;
    }
    navigate("/programOverlay", { state: { currentProgram: programsState[programId] } });
  };

  const addProgram = async () => { // Make async
    const auth = getAuth();
    const user = auth.currentUser;

    if (!user || !user.email) { // Check user and email
      toast.error("You must be logged in to add a program.");
      logEvent(analytics, "notification_received", { notification_type: "add_program_auth_fail" });
      return;
    }

    // **Improved ID Generation (Example: Max numeric ID + 1)**
    const existingIds = Object.keys(programsState).map(id => parseInt(id, 10)).filter(id => !isNaN(id));
...
    setProgramsState(newState);
    ...
    try {
        ...
        toast.error(`Error adding program: ${error.message || 'Please try again.'}`); // Provide error feedback
...
  };
 ...
 const removeProgram = async (programId) => { // Make async
    const auth = getAuth();
    const user = auth.currentUser;

    if (!user || !user.email) { // Check user and email
      toast.error("You must be logged in to remove a program.");
      logEvent(analytics, "notification_received", { notification_type: "remove_program_auth_fail" });
      return;
    }

    // Get name for confirmation dialog, handle if program doesn't exist unexpectedly
    const programNameToDelete = programsState[programId]?.name || `Program ${programId}`;
    const confirmDelete = window.confirm(`Are you sure you want to delete "${programNameToDelete}"?`);
    ...
    try {
        ...
        toast.error(`Error deleting program: ${error.message || 'Please try again.'}`); // Error feedback
        logEvent(analytics, "program_remove_failed", { program_id: programId, error: error.message });
        // **Rollback UI on failure**
...
        {programsState && typeof programsState === 'object' && Object.entries(programsState).map(([programId, programData]) => {
          // Basic check for valid programData structure to prevent render errors
          if (!programData || typeof programData.name !== 'string' || !Array.isArray(programData.list)) {
             console.warn(`Invalid program data structure for ID: ${programId}`, programData);
